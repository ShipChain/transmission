...
class ShipmentCreateSerializer(ShipmentSerializer):
    ...
    def validate_afts_tracking(self, afts_tracking):
        response = requests.post(f'{settings.AFTERSHIP_URL}couriers/detect',
                                 headers={'aftership-api-key': settings.AFTERSHIP_API_KEY},
                                 json={'tracking': {'tracking_number': afts_tracking}})
        if not response.ok:
            raise serializers.ValidationError('Invalid afts_tracking value')

        return afts_tracking
...
class ShipmentUpdateSerializer(ShipmentSerializer):
    ...
    class Meta:
        model = Shipment
        exclude = (('owner_id', 'version', 'afts_tracking',
                   ...
                   if settings.PROFILES_ENABLED else ('version', 'background_data_hash_interval',
                                                      'afts_tracking', 'manual_update_hash_interval',
...
class ShipmentVaultSerializer(NullableFieldsMixin, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        exclude = ('owner_id', 'storage_credentials_id', 'background_data_hash_interval',
                   ...
                   'afts_tracking', 'carrier_abbv', 'arrival_est')
