...
def shipments_with_device(user, user2, devices, mocked_engine_rpc, mocked_iot_api):
    list_shipment = []
    owner_id = user.token.get('organization_id')
    third_owner_id = user2.token.get('organization_id')
...
def test_filter_shipment_device_location(api_client, shipment_tracking_data, json_asserter):
    ...
    action_response = api_client.post(shipment_action_url, data=pickup_action, format='json')
    json_asserter.HTTP_200(action_response)

    action_response = api_client.post(shipment2_action_url, data=pickup_action, format='json')
    json_asserter.HTTP_200(action_response)

    action_response = api_client.post(shipment2_action_url, data=arrival_action, format='json')
    json_asserter.HTTP_200(action_response)

    in_bbox_url = f'{url}?in_bbox={",".join([str(x) for x in BBOX])}'

    # The primary user has exactly (NUM_DEVICES - 3) inside Bbox
    response = api_client.get(in_bbox_url)
    response_data = response.json()
    json_asserter.HTTP_200(response, vnd=True, is_list=True)
    ...
    response = api_client.get(in_transit_url)
    response_data = response.json()
    json_asserter.HTTP_200(response, vnd=True, is_list=True)
    ...
    response = api_client.get(list_state_url)
    response_data = response.json()
    json_asserter.HTTP_200(response, vnd=True, is_list=True)
    ...
    response = api_client.get(has_location_url)
    response_data = response.json()
    json_asserter.HTTP_200(response, vnd=True, is_list=True)
    assert response_data['meta']['pagination']['count'] == 1
