...
class TransactionReceiptTestCase(APITestCase):
    ...
    def test_transaction_list(self):
        ...
        listener = DummyEthListener(id='FAKE_LISTENER_ID')
        ...
        url = reverse('transactionreceipts-list', kwargs={'version': 'v1'})

        # Request without wallet_address query field should fail
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # request for specific transaction recepits should only return ones with that from_address
